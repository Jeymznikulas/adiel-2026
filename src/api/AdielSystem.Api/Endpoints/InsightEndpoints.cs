...
public static class InsightEndpoints
{
    public static RouteGroupBuilder MapInsightEndpoints(this RouteGroupBuilder group)
    {
        ...
        insights.MapGet("/dashboard", async (int? trendMonths, InsightsService service, CancellationToken token) => Results.Ok(await service.GetDashboardAsync(trendMonths ?? 6, token)))
        ...
        insights.MapGet("/activity", async (string? search, string? module, string? action, DateOnly? date, string? sort, int? page, int? pageSize, InsightsService service, CancellationToken token) => Results.Ok(await service.ListActivityAsync(search, module, action, date, sort, page ?? 1, pageSize ?? 30, token)))
