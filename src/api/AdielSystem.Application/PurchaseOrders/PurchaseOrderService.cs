...
{
    public Task<PurchaseOrderPageDto> ListAsync(string? search, Guid? supplierId, string? status, bool archivedOnly, int page, int pageSize, CancellationToken token) => page <= 0 || pageSize is < 1 or > 100 ? throw new RequestValidationException("Page must be greater than zero and page size must be between 1 and 100.") : repository.ListAsync(search?.Trim() ?? string.Empty, supplierId, string.IsNullOrWhiteSpace(status) ? null : status.Trim(), archivedOnly, page, pageSize, token);
