...
{
    public async Task<DashboardDto> GetDashboardAsync(int trendMonths, DateOnly today, CancellationToken token)
    {
        ...
        var trend = await ReadTrendAsync(connection, trendMonths, token);
    ...
    {
        await using var connection = await dataSource.OpenConnectionAsync(token);
        const string where = "where (@module='' or module=@module) and (@action='' or action=@action) and (@date is null or occurred_at::date=@date) and (@search='' or entity ilike @search escape '\\' or description ilike @search escape '\\' or actor_name ilike @search escape '\\')";
    ...
    private static async Task<IReadOnlyList<DashboardTrendDto>> ReadTrendAsync(NpgsqlConnection connection,int trendMonths,CancellationToken token)
    { await using var command=connection.CreateCommand(); command.CommandText="""with months as(select generate_series(date_trunc('month',current_date)-(@months-1)*interval '1 month',date_trunc('month',current_date),interval '1 month')::date month_start) select to_char(m.month_start,'YYYY-MM'),to_char(m.month_start,'Mon'),coalesce((select sum(q.total_amount) from public.quotations q where q.status='Approved' and q.archived_at is null and q.deleted_at is null and q.quotation_date>=m.month_start and q.quotation_date<(m.month_start+interval '1 month')::date),0),coalesce((select sum(e.amount) from public.expenses e where e.status<>'Cancelled' and e.archived_at is null and e.deleted_at is null and e.expense_date>=m.month_start and e.expense_date<(m.month_start+interval '1 month')::date),0),coalesce((select sum(q.subtotal_amount) from public.quotations q where q.status='Approved' and q.archived_at is null and q.deleted_at is null and q.quotation_date>=m.month_start and q.quotation_date<(m.month_start+interval '1 month')::date),0)-coalesce((select sum(e.amount) from public.expenses e where e.status<>'Cancelled' and e.archived_at is null and e.deleted_at is null and e.expense_date>=m.month_start and e.expense_date<(m.month_start+interval '1 month')::date),0) from months m order by m.month_start""";command.Parameters.AddWithValue("months",trendMonths);var rows=new List<DashboardTrendDto>();await using var reader=await command.ExecuteReaderAsync(token);while(await reader.ReadAsync(token))rows.Add(new(reader.GetString(0),reader.GetString(1),reader.GetDecimal(2),reader.GetDecimal(3),reader.GetDecimal(4)));return rows; }
    ...
    private static void AddActivityParameters(NpgsqlCommand command,ActivityQuery query){command.Parameters.AddWithValue("module",query.Module??string.Empty);command.Parameters.AddWithValue("action",query.Action??string.Empty);command.Parameters.AddWithValue("date",NpgsqlDbType.Date,(object?)query.Date??DBNull.Value);command.Parameters.AddWithValue("search",string.IsNullOrWhiteSpace(query.Search)?string.Empty:"%"+EscapeLike(query.Search)+"%");}
