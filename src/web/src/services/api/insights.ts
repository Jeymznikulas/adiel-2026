...
export type ActivityEntry = { id: string; recordId: string | null; timestamp: string; module: 'Settings' | 'Tasks' | 'Items' | 'Expenses' | 'Suppliers' | 'Clients' | 'Quotations' | 'Purchase Orders' | 'Statements of Account'; action: string; entity: string; description: string; actor: string; tone: 'success' | 'info' | 'warning' | 'danger'; amount: number | null; status: string | null }
...
export const getDashboard = (trendMonths: 6 | 12, from?: string, to?: string) => apiRequest<Dashboard>('/dashboard?' + parameters({ trendMonths, from, to }))
...
export const listActivity = (query: { search?: string; module?: string; action?: string; date?: string; recordId?: string; sort?: string; page?: number; pageSize?: number } = {}) => apiRequest<ActivityPage>('/activity?' + parameters({ ...query, page: query.page ?? 1, pageSize: query.pageSize ?? 30 }))
