...
export function SummarySurface({ children, className = '', ...props }: SummarySurfaceProps) {
  return (
    <section
      className={`relative isolate overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_14px_45px_-28px_rgba(0,20,76,0.3)] sm:p-6 ${className}`}
      {...props}
    >
      <span className="pointer-events-none absolute inset-x-8 top-0 -z-10 h-px bg-[linear-gradient(90deg,transparent,rgba(253,77,0,0.45),rgba(0,20,76,0.18),transparent)]" aria-hidden="true" />
