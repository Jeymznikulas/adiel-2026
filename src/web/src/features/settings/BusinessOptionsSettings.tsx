...
export function BusinessOptionsSettings({ initialTab }: { initialTab: BusinessSettingsTab }) {
  ...
  useEffect(() => {
    if (!editingId) return
    setEditingEmail(options.find((option) => option.id === editingId)?.email ?? '')
  }, [editingId, options])

  function chooseTab(tab: BusinessSettingsTab) {
    setActiveTab(tab); setNewName(''); setNewEmail(''); setEditingId(null); setError(''); setMessage('')
  ...
  function add(event: FormEvent<HTMLFormElement>) {
    ...
    if (detail.type === 'task_assignee' && !email) { setError('Enter the email that should receive Google Calendar invitations.'); return }
    void run(async () => { const created = await createBusinessOption(detail.type, name, email || undefined); setOptions((current) => [...current, created]); setNewName(''); setNewEmail(''); setMessage(`${created.name} added`) })
  }
  ...
  function rename(event: FormEvent<HTMLFormElement>, option: BusinessOption) {
    ...
    if (!name) { setError(`Enter a ${detail.singular} name.`); return }
    if (detail.type === 'task_assignee' && !email) { setError('Enter the email that should receive Google Calendar invitations.'); return }
    void run(async () => { const saved = await renameBusinessOption(option.id, name, option.version, email || undefined); setOptions((current) => current.map((entry) => entry.id === saved.id ? saved : entry)); setEditingId(null); setMessage(`${saved.name} updated`) })
  }
  ...
  return <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_18px_48px_-36px_rgba(0,20,76,0.45)]">
    ...
    <div className="grid lg:grid-cols-[15rem_1fr]">
      ...
      <div className="min-w-0 p-5 sm:p-6">
        ...
        <form className={`mt-5 grid gap-2 rounded-2xl border border-slate-200 bg-slate-50/55 p-3 ${detail.type === 'task_assignee' ? 'sm:grid-cols-[1fr_1.25fr_auto]' : 'sm:grid-cols-[1fr_auto]'}`} onSubmit={add}>
          <label className="sr-only" htmlFor="business-option-name">New option</label>
          <input className="h-10 min-w-0 rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-medium text-brand-blue outline-none placeholder:text-slate-300 focus:border-brand-blue/40" id="business-option-name" value={newName} onChange={(event) => { setNewName(event.target.value); setError(''); setMessage('') }} placeholder={`Add ${detail.singular}`} />
          {detail.type === 'task_assignee' ? <><label className="sr-only" htmlFor="business-option-email">Calendar email</label><input className="h-10 min-w-0 rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-medium text-brand-blue outline-none placeholder:text-slate-300 focus:border-brand-blue/40" id="business-option-email" type="email" value={newEmail} onChange={(event) => { setNewEmail(event.target.value); setError(''); setMessage('') }} placeholder="Google Calendar email" required /></> : null}
          <button className="h-10 rounded-xl bg-brand-blue px-4 text-xs font-bold text-white transition hover:-translate-y-0.5 disabled:opacity-40" type="submit" disabled={isBusy || !newName.trim() || detail.type === 'task_assignee' && !newEmail.trim()}>Add</button>
        </form>
        ...
        {detail.type === 'task_assignee' && editingId ? <div className="mt-3 rounded-xl border border-blue-100 bg-blue-50/50 p-3"><label className="mb-1.5 block text-[9px] font-bold uppercase tracking-wider text-slate-500" htmlFor="editing-assignee-email">Google Calendar email</label><input className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-brand-blue outline-none focus:border-brand-blue/40" id="editing-assignee-email" type="email" value={editingEmail} onChange={(event) => setEditingEmail(event.target.value)} required /><p className="mt-1.5 text-[9px] text-slate-400">Saving the assignee name also saves this invitation address.</p></div> : null}
