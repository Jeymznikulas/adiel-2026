...
type SettingsSection = 'company' | 'documents' | 'numbering' | 'payments' | 'options' | 'calendar' | 'backup'

function initialSection(): SettingsSection {
  const section = new URLSearchParams(window.location.search).get('section')
  return section === 'documents' || section === 'numbering' || section === 'payments' || section === 'options' || section === 'calendar' || section === 'backup' ? section : 'company'
}
...
export function SettingsPage() {
  ...
  return <div className="space-y-5 animate-[content-enter_320ms_cubic-bezier(0.22,1,0.36,1)]">
    ...
    <SummarySurface className="space-y-5" aria-label="Settings summary">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Manage company details, document defaults, integrations, reusable options, and protected database backups.</p>
      </div>
      <nav className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7 [&>button]:!min-w-0" aria-label="Settings sections">
        ...
        <button className={`flex min-w-0 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left transition-all duration-200 sm:min-w-40 ${activeSection === 'calendar' ? 'border-brand-blue/20 bg-blue-50/70 text-brand-blue shadow-[0_10px_24px_-20px_rgba(0,20,76,0.55)]' : 'border-slate-200/80 bg-white text-slate-500 hover:-translate-y-0.5 hover:border-brand-blue/15 hover:text-brand-blue'}`} type="button" onClick={() => selectSection('calendar')} aria-current={activeSection === 'calendar' ? 'page' : undefined}><span className={`grid size-9 shrink-0 place-items-center rounded-xl ${activeSection === 'calendar' ? 'bg-brand-blue text-white' : 'bg-slate-100 text-slate-400'}`}><Icon path="M8 2v4M16 2v4M3 9h18M5 4h14a2 2 0 0 1 2 2v14H3V6a2 2 0 0 1 2-2Z" /></span><span className="min-w-0"><span className="block truncate text-xs font-bold">Calendar</span><span className="mt-0.5 block truncate text-[9px] text-slate-400">Google invitations</span></span></button>
        <button className={`flex min-w-0 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left transition-all duration-200 sm:min-w-40 ${activeSection === 'backup' ? 'border-brand-blue/20 bg-blue-50/70 text-brand-blue shadow-[0_10px_24px_-20px_rgba(0,20,76,0.55)]' : 'border-slate-200/80 bg-white text-slate-500 hover:-translate-y-0.5 hover:border-brand-blue/15 hover:text-brand-blue'}`} type="button" onClick={() => selectSection('backup')} aria-current={activeSection === 'backup' ? 'page' : undefined}><span className={`grid size-9 shrink-0 place-items-center rounded-xl ${activeSection === 'backup' ? 'bg-brand-blue text-white' : 'bg-slate-100 text-slate-400'}`}><Icon path="M12 3c-4.4 0-8 1.3-8 3s3.6 3 8 3 8-1.3 8-3-3.6-3-8-3Zm-8 3v6c0 1.7 3.6 3 8 3s8-1.3 8-3V6M4 12v6c0 1.7 3.6 3 8 3s8-1.3 8-3v-6" /></span><span className="min-w-0"><span className="block truncate text-xs font-bold">Backup</span><span className="mt-0.5 block truncate text-[9px] text-slate-400">Database recovery</span></span></button>
      </nav>
    </SummarySurface>
    ...
    </form> : activeSection === 'calendar' ? <GoogleCalendarSettings /> : activeSection === 'backup' ? <DatabaseBackupSettings /> : <BusinessOptionsSettings initialTab={initialBusinessTab()} />}
