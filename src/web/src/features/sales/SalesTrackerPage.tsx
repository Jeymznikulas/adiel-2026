...
export function SalesTrackerPage() {
  ...
  const salesSortOptions = [
    { value: 'newest', label: 'Newest first', getValue: (row: SalesRow) => row.quotation.dateCreated, direction: 'desc' as const },
    { value: 'oldest', label: 'Oldest first', getValue: (row: SalesRow) => row.quotation.dateCreated, direction: 'asc' as const },
    { value: 'amount', label: 'Highest sales amount', getValue: (row: SalesRow) => row.quotation.totalAmount, direction: 'desc' as const },
    { value: 'client', label: 'Client A-Z', getValue: (row: SalesRow) => row.quotation.clientName, direction: 'asc' as const },
  ]
  const salesTable = useTableView({ rows: matchingRows, storageKey: 'sales.table', sortOptions: salesSortOptions })
  ...
  return <div className="space-y-5 animate-[content-enter_320ms_cubic-bezier(0.22,1,0.36,1)]">
    <SummarySurface className="relative overflow-hidden">
      ...
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><div className="flex items-center gap-2"><span className="h-px w-6 bg-brand-orange" /><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-brand-orange">Approved sales</p></div><h2 className="mt-3 text-2xl font-extrabold tracking-[-0.04em] text-brand-blue sm:text-3xl">Sales</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">See sales, statements, payments, expenses, and profit from approved quotations in one place.</p></div><div className="w-full max-w-2xl"><p className="mb-2 text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400">Date range</p><div className="inline-flex w-full rounded-xl border border-slate-200 bg-slate-50 p-1 sm:w-auto">{periodModes.map((mode) => <button className={periodMode === mode ? 'flex-1 rounded-lg bg-white px-3 py-2 text-[10px] font-extrabold text-brand-blue shadow-sm ring-1 ring-slate-200 sm:flex-none sm:px-4' : 'flex-1 rounded-lg px-3 py-2 text-[10px] font-bold text-slate-400 transition hover:bg-white/70 hover:text-brand-blue sm:flex-none sm:px-4'} type="button" aria-pressed={periodMode === mode} onClick={() => setPeriodMode(mode)} key={mode}>{mode}</button>)}</div>{periodMode === 'Custom' ? <div className="mt-2 grid gap-2 sm:grid-cols-2"><div><p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.08em] text-slate-400">From</p><AnimatedDatePicker value={customFrom} onChange={(from) => { setCustomFrom(from); if (from > customTo) setCustomTo(from) }} ariaLabel="Custom report start date" max={customTo} size="filter" required /></div><div><p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.08em] text-slate-400">To</p><AnimatedDatePicker value={customTo} onChange={(to) => { setCustomTo(to); if (to < customFrom) setCustomFrom(to) }} ariaLabel="Custom report end date" min={customFrom} size="filter" required /></div></div> : <div className="mt-2 flex items-center gap-2"><button className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-400 transition hover:border-brand-blue/20 hover:text-brand-blue" type="button" onClick={() => setAnchorDate((current) => shiftPeriod(periodMode, current, -1))} aria-label="Previous date range"><Icon path="m15 18-6-6 6-6" /></button><div className="min-w-0 flex-1"><AnimatedDatePicker value={anchorDate} onChange={setAnchorDate} ariaLabel="Choose date" triggerLabel={activePeriodLabel} size="filter" required /></div><button className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-400 transition hover:border-brand-blue/20 hover:text-brand-blue" type="button" onClick={() => setAnchorDate((current) => shiftPeriod(periodMode, current, 1))} aria-label="Next date range"><Icon path="m9 18 6-6-6-6" /></button><button className="h-10 shrink-0 rounded-xl border border-orange-100 bg-orange-50 px-3 text-[10px] font-bold text-brand-orange transition hover:bg-orange-100" type="button" onClick={() => setAnchorDate(dateKey(new Date()))}>Today</button></div>}</div></div>
      <div className="relative mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><Metric label="Estimated revenue" value={formatPeso(estimatedRevenue)} detail={`${periodRows.length} quotations · net of VAT`} tone="blue" /><Metric label="Estimated cost" value={formatPeso(estimatedCost)} detail="Quotation item costs" tone="slate" /><Metric label="Actual expenses" value={formatPeso(actualExpenses)} detail={`${expenses.filter((expense) => periodQuotationIds.has(expense.quotationId)).length} linked entries`} tone="violet" /><Metric label="Estimated profit" value={formatPeso(totalProfit)} detail={`${profitMargin.toFixed(1)}% margin`} tone="green" /><Metric label="Actual profit" value={formatPeso(actualProfit)} detail="Revenue less actual expenses" tone={actualProfit >= 0 ? 'cyan' : 'orange'} /><Metric label="Profit difference" value={`${profitVariance >= 0 ? '+' : ''}${formatPeso(profitVariance)}`} detail={profitVariance >= 0 ? 'Above estimate' : 'Below estimate'} tone={profitVariance >= 0 ? 'green' : 'orange'} /></div>
    </SummarySurface>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.55fr_0.45fr]">
      ...
      <aside className="rounded-[1.5rem] bg-[linear-gradient(145deg,#00113f,#073078)] p-5 text-white shadow-[0_20px_48px_-30px_rgba(0,20,76,0.75)] sm:p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-white/10"><Icon path="M3 3v18h18M7 16l4-5 3 3 6-8" /></span><div><h3 className="text-sm font-extrabold">Summary</h3><p className="mt-0.5 text-[10px] text-white/45">For the selected period</p></div></div><div className="mt-6 space-y-4"><Snapshot label="Approved sales" value={formatPeso(totalSales)} /><Snapshot label="Billed sales" value={formatPeso(billedSales)} /><Snapshot label="Collections" value={formatPeso(collected)} /><Snapshot label="Receivables" value={formatPeso(receivables)} /><Snapshot label="Actual profit rate" value={`${estimatedRevenue ? ((actualProfit / estimatedRevenue) * 100).toFixed(1) : '0.0'}%`} /></div><div className="mt-6 border-t border-white/15 pt-5"><p className="text-[9px] leading-4 text-white/45">Actual profit uses the expenses linked to each approved quotation.</p></div></aside>
    </div>
    ...
    <TableControls tableId="sales-register" storageKey="sales.table" columns={[{ index: 1, label: 'Quotation date' }, { index: 2, label: 'Quotation', required: true }, { index: 3, label: 'Client and project' }, { index: 4, label: 'Items' }, { index: 5, label: 'Sales amount' }, { index: 6, label: 'Estimated profit' }, { index: 7, label: 'Billing' }, { index: 8, label: 'Collection' }, { index: 9, label: 'SOA' }, { index: 10, label: 'Action', required: true }]} sortKey={salesTable.sortKey} sortOptions={salesSortOptions} onSortChange={salesTable.setSortKey} page={salesTable.page} pageCount={salesTable.pageCount} pageSize={salesTable.pageSize} onPageChange={salesTable.setPage} onPageSizeChange={salesTable.setPageSize} total={salesTable.total} />
    <div id="sales-register">
    <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]"><header className="border-b border-slate-100 p-4 sm:p-5"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><h3 className="text-sm font-extrabold text-brand-blue">All sales</h3><p className="mt-1 text-[10px] text-slate-400">Each approved quotation appears once. Statements do not add duplicate sales.</p></div><div className="flex flex-col gap-2 sm:flex-row"><div className="relative"><Icon className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-300" path="m21 21-4.35-4.35M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0Z" /><input className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50/70 pl-10 pr-4 text-xs font-medium text-brand-blue outline-none focus:border-brand-blue/40 sm:w-64" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search quotation, client, project..." /></div><AnimatedDropdown className="sm:min-w-36" size="filter" value={billingFilter} options={billingOptions} onChange={setBillingFilter} ariaLabel="Filter billing status" /><AnimatedDropdown className="sm:min-w-40" size="filter" value={collectionFilter} options={collectionOptions} onChange={setCollectionFilter} ariaLabel="Filter collection status" /></div></div></header>
