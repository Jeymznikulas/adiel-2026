...
export function CollectionsPage() {
  ...
  return <div className="space-y-5 animate-[content-enter_320ms_cubic-bezier(0.22,1,0.36,1)]">
    ...
    <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]">
      <header className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><h3 className="text-sm font-extrabold text-brand-blue">Open collection schedule</h3><p className="mt-1 text-[10px] text-slate-400">Sorted by oldest due date first</p></div><div className="flex flex-col gap-2 sm:flex-row"><input className="app-control h-10 rounded-xl border border-slate-200 bg-slate-50/60 px-3.5 text-xs font-medium text-brand-blue outline-none focus:border-brand-blue/30 sm:w-64" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search client or SOA..." /><AnimatedDropdown className="sm:min-w-40" size="filter" value={filter} options={filters} onChange={setFilter} ariaLabel="Filter collection status" /></div></header>
      {visibleRows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-left"><thead><tr className="bg-slate-50/70 text-[9px] font-bold uppercase tracking-[0.08em] text-slate-400"><th className="px-5 py-3.5">Client / SOA</th><th className="px-5 py-3.5">Schedule</th><th className="px-5 py-3.5">Due date</th><th className="px-5 py-3.5 text-right">Principal due</th><th className="px-5 py-3.5 text-right">Late charge</th><th className="px-5 py-3.5 text-right">Total due</th><th className="px-5 py-3.5">Status</th><th className="px-5 py-3.5 text-right">Actions</th></tr></thead><tbody>{visibleRows.map((row) => <tr className="border-t border-slate-100 transition hover:bg-blue-50/25" key={row.id}><td className="px-5 py-4"><p className="text-xs font-extrabold text-slate-700">{row.statement.clientName}</p><p className="mt-1 font-mono text-[9px] font-bold text-brand-blue">{row.statement.soaNumber}</p></td><td className="px-5 py-4 text-xs font-semibold text-slate-500">{row.schedule.label}</td><td className="px-5 py-4"><p className="text-xs font-semibold text-slate-600">{formatDate(row.schedule.dueDate)}</p>{row.schedule.daysLate ? <p className="mt-1 text-[9px] font-bold text-red-500">{row.schedule.daysLate} days late</p> : null}</td><td className="px-5 py-4 text-right text-xs font-extrabold tabular-nums text-amber-700">{formatPeso(row.schedule.balance)}</td><td className="px-5 py-4 text-right"><p className="text-xs font-extrabold tabular-nums text-red-600">{row.lateCharge ? formatPeso(row.lateCharge.balance) : '—'}</p>{row.suggestedLateCharge > 0 ? <p className="mt-1 text-[9px] font-bold text-violet-600">Suggested {formatPeso(row.suggestedLateCharge)}</p> : null}</td><td className="px-5 py-4 text-right text-xs font-extrabold tabular-nums text-brand-blue">{formatPeso(row.totalDue)}</td><td className="px-5 py-4"><span className={`inline-flex rounded-lg border px-2.5 py-1 text-[9px] font-bold ${statusTone(row.schedule.status)}`}>{row.schedule.status}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-2">{row.schedule.status === 'Overdue' || row.lateCharge ? <Button size="small" variant="secondary" leadingIcon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z" /></svg>} onClick={() => navigate(`/statement-of-account/${row.statement.id}?charge=${encodeURIComponent(row.schedule.id)}`)}>{row.lateCharge ? 'Edit charge' : 'Review charge'}</Button> : null}<Button size="small" variant="primary" leadingIcon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>} onClick={() => navigate(`/statement-of-account/${row.statement.id}?pay=1`)}>Record payment</Button></div></td></tr>)}</tbody></table></div> : <div className="grid min-h-64 place-items-center p-8 text-center"><div><p className="text-sm font-extrabold text-brand-blue">No matching collections</p><p className="mt-2 text-xs text-slate-400">{rows.length ? 'Adjust the search or collection status filter to see more entries.' : 'Open SOA schedule balances will appear here automatically.'}</p>{rows.length ? <Button className="mt-4" size="small" variant="secondary" onClick={() => { setSearch(''); setFilter('All open') }}>Clear filters</Button> : null}</div></div>}
    </section>
  </div>
}
