...
function ProductPhoto({ photo, name, itemId, variantId }: { photo: string; name: string; itemId: string; variantId: string }) {
  return photo
    ? <PrivateImage className="size-11 shrink-0 rounded-xl border border-slate-100 object-cover" target="items" entityId={itemId || undefined} variantId={variantId || undefined} objectPath={photo} alt="" />
            ...
            <table className="w-full min-w-[940px] table-fixed text-left">
              ...
              <tbody>{quotation.items.map((item, index) => {
                ...
                return <tr className={`border-t border-slate-100 transition-colors ${decision === 'approved' ? 'bg-emerald-50/30' : decision === 'not-approved' ? 'bg-red-50/35' : 'hover:bg-blue-50/25'}`} key={item.id}>
                  ...
                  <td className="px-4 py-3"><div className="flex min-w-0 items-center gap-3"><ProductPhoto itemId={item.itemId} variantId={item.variantId} photo={item.photo} name={item.itemName} /><div className="min-w-0"><p className="truncate text-xs font-extrabold text-slate-700">{item.itemName}</p><p className="mt-1 truncate text-[9px] text-slate-400">{item.productCode || 'No product code'}{item.variantLabel ? ` · ${item.variantLabel}` : ''}</p></div></div></td>
