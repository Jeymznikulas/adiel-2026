...
function ProductPhoto({ photo, name, itemId, variantId, className = 'size-11' }: { photo: string; name: string; itemId?: string; variantId?: string; className?: string }) {
  const entityId = itemId || (variantId ? undefined : photo.split('/')[2])
  return photo ? <PrivateImage className={`${className} shrink-0 object-cover`} target="items" entityId={entityId} variantId={variantId} objectPath={photo} alt="" /> : <span className={`${className} grid shrink-0 place-items-center bg-[linear-gradient(145deg,#eef3fb,#e3eaf5)] text-[10px] font-extrabold text-brand-blue`}>{name.split(/\s+/).map((part) => part[0]).filter(Boolean).join('').slice(0, 2).toUpperCase() || 'IT'}</span>
}
...
function LoadingBlock({ className = '' }: { className?: string }) {
  return <span className={`block animate-pulse rounded-lg bg-slate-200/80 motion-reduce:animate-none ${className}`} aria-hidden="true" />
}

function LoadingField({ className = '', tall = false }: { className?: string; tall?: boolean }) {
  return <div className={className} aria-hidden="true"><LoadingBlock className="mb-2 h-2.5 w-20" /><LoadingBlock className={`${tall ? 'h-12' : 'h-11'} w-full rounded-xl`} /></div>
}

function LoadingSectionTitle({ title, detail, tone = 'blue' }: { title: string; detail: string; tone?: 'blue' | 'violet' | 'emerald' }) {
  const toneClass = tone === 'emerald' ? 'bg-emerald-50' : tone === 'violet' ? 'bg-violet-50' : 'bg-blue-50'
  return <div className="mb-4 flex items-center gap-3"><LoadingBlock className={`size-8 rounded-xl ${toneClass}`} /><div><h3 className="text-xs font-extrabold text-brand-blue">{title}</h3><p className="text-[10px] text-slate-400">{detail}</p></div></div>
}

function QuotationEditorLoading({ onBack }: { onBack: () => void }) {
  return <section className="mx-auto flex min-h-[calc(100svh-9rem)] w-full max-w-7xl flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white animate-[content-enter_280ms_cubic-bezier(0.22,1,0.36,1)]" role="status" aria-live="polite" aria-busy="true">
    <span className="sr-only">Preparing quotation editor</span>
    <div className="h-1 shrink-0 overflow-hidden bg-blue-50" aria-hidden="true"><div className="h-full w-full animate-pulse bg-[linear-gradient(90deg,#00144c,#5b7fe8,#fd4d00)] motion-reduce:animate-none" /></div>
    <header className="flex items-start gap-3 border-b border-slate-100 bg-white/95 px-4 py-5 sm:px-6 lg:px-8">
      <button className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-bold text-brand-blue transition hover:bg-blue-50" type="button" onClick={onBack}><Icon className="size-3.5" path="m15 18-6-6 6-6" /><span className="hidden sm:inline">Back to quotations</span></button>
      <div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-brand-orange">Quotation</p><h2 className="mt-1.5 text-xl font-bold tracking-[-0.03em] text-brand-blue">Edit quotation</h2><p className="mt-1 text-xs text-slate-400">Client, project, prices, and expected profit</p></div>
      <span className="inline-flex h-9 shrink-0 items-center gap-2 rounded-xl border border-blue-100 bg-blue-50/80 px-3 text-[10px] font-bold text-brand-blue"><span className="size-2 animate-pulse rounded-full bg-brand-orange motion-reduce:animate-none" />Loading</span>
    </header>

    <div className="flex-1 px-4 py-5 sm:px-6 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-2">
        <section>
          <LoadingSectionTitle title="Client information" detail="Selected from Clients" />
          <div className="grid gap-4 sm:grid-cols-2"><LoadingField className="sm:col-span-2" tall /><LoadingField /><LoadingField /></div>
        </section>
        <section>
          <LoadingSectionTitle title="Quotation details" detail="Number, date, and approval status" tone="violet" />
          <div className="grid gap-4 sm:grid-cols-2"><LoadingField /><LoadingField /><LoadingField className="sm:col-span-2" /></div>
        </section>
      </div>

      <section className="mt-6 border-t border-slate-100 pt-5" aria-hidden="true">
        <div className="grid gap-4 lg:grid-cols-2"><LoadingField /><LoadingField /><LoadingField className="lg:col-span-2" /></div>
      </section>

      <section className="mt-6 border-t border-slate-100 pt-5" aria-hidden="true">
        <div className="flex items-center justify-between gap-3"><div><h3 className="text-xs font-extrabold text-brand-blue">Quotation items</h3><p className="mt-1 text-[10px] text-slate-400">Prices and costs come from Items.</p></div><LoadingBlock className="h-9 w-28 rounded-xl" /></div>
        <div className="mt-4 space-y-3">{Array.from({ length: 3 }, (_, index) => <article className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/55 p-4 lg:grid-cols-[2rem_1.45fr_1fr_0.5fr_0.72fr_0.72fr_auto] lg:items-end" key={index}><LoadingBlock className="size-8 rounded-lg" /><div><LoadingBlock className="mb-2 h-2.5 w-14" /><div className="flex h-11 items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-2"><LoadingBlock className="size-8 shrink-0 rounded-lg" /><div className="min-w-0 flex-1 space-y-1.5"><LoadingBlock className={`h-2.5 ${index === 1 ? 'w-3/4' : 'w-1/2'}`} /><LoadingBlock className="h-2 w-1/3" /></div></div></div><LoadingField /><LoadingField /><LoadingField /><div><LoadingBlock className="mb-2 h-2.5 w-14" /><div className="flex h-11 flex-col justify-center gap-1.5"><LoadingBlock className="h-3 w-20" /><LoadingBlock className="h-2 w-14" /></div></div><LoadingBlock className="size-9 rounded-lg" /></article>)}</div>
      </section>

      <section className="mt-6 border-t border-slate-100 pt-5" aria-hidden="true">
        <LoadingSectionTitle title="Taxes & additional fees" detail="Build the final quoted amount with a clear pricing breakdown." tone="emerald" />
        <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-3"><article className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50/55 p-4"><div className="space-y-2"><LoadingBlock className="h-3.5 w-20" /><LoadingBlock className="h-2.5 w-64 max-w-full" /></div><div className="flex items-center gap-3"><LoadingBlock className="h-3 w-20" /><LoadingBlock className="h-7 w-12 rounded-full" /></div></article><article className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><div className="space-y-2"><LoadingBlock className="h-3.5 w-20" /><LoadingBlock className="h-2.5 w-52 max-w-full" /></div><LoadingBlock className="h-9 w-24 rounded-xl" /></div><div className="mt-3 rounded-xl border border-dashed border-slate-200 px-3 py-4"><LoadingBlock className="mx-auto h-2.5 w-32" /></div></article></div>
          <aside className="self-start rounded-2xl bg-[linear-gradient(145deg,#00113f,#073078)] p-5 shadow-[0_18px_38px_-24px_rgba(0,20,76,0.75)]"><div className="h-2.5 w-28 animate-pulse rounded bg-white/20 motion-reduce:animate-none" /><div className="mt-4 space-y-3">{Array.from({ length: 2 }, (_, index) => <div className="flex justify-between gap-4" key={index}><div className="h-2.5 w-20 animate-pulse rounded bg-white/15 motion-reduce:animate-none" /><div className="h-3 w-20 animate-pulse rounded bg-white/30 motion-reduce:animate-none" /></div>)}</div><div className="mt-4 flex items-end justify-between border-t border-white/15 pt-4"><div className="h-2.5 w-20 animate-pulse rounded bg-white/20 motion-reduce:animate-none" /><div className="h-7 w-32 animate-pulse rounded-lg bg-white/30 motion-reduce:animate-none" /></div></aside>
        </div>
      </section>

      <section className="mt-6 grid gap-3 border-t border-slate-100 pt-5 sm:grid-cols-3" aria-hidden="true">{['bg-slate-50', 'bg-emerald-50/65', 'bg-violet-50/65'].map((tone, index) => <div className={`rounded-2xl p-4 ${tone}`} key={tone}><LoadingBlock className="h-2.5 w-24" /><LoadingBlock className={`mt-2 h-5 ${index === 2 ? 'w-16' : 'w-28'}`} /></div>)}</section>

      <section className="mt-6 border-t border-slate-100 pt-5" aria-hidden="true"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-xs font-extrabold text-brand-blue">Additional document details</h3><p className="mt-1 text-[10px] text-slate-400">Optional content saved with this document and included in its PDF.</p></div><div className="flex gap-2"><LoadingBlock className="h-9 w-24 rounded-xl" /><LoadingBlock className="h-9 w-40 rounded-xl" /></div></div></section>
    </div>

    <footer className="sticky bottom-0 z-10 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-16px_36px_-30px_rgba(0,20,76,0.55)] backdrop-blur sm:px-6 lg:px-8" aria-hidden="true"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap items-center gap-x-5 gap-y-2">{Array.from({ length: 2 }, (_, index) => <div className="space-y-1.5" key={index}><LoadingBlock className="h-2 w-14" /><LoadingBlock className="h-3 w-20" /></div>)}<div className="border-l border-slate-200 pl-5"><LoadingBlock className="h-2 w-10" /><LoadingBlock className="mt-1.5 h-4 w-28" /></div></div><div className="flex justify-end gap-2"><LoadingBlock className="h-10 w-20 rounded-xl" /><LoadingBlock className="h-10 w-24 rounded-xl" /><div className="h-10 w-36 animate-pulse rounded-xl bg-brand-blue/80 motion-reduce:animate-none" /></div></div></footer>
  </section>
}

function QuotationDetailLoading({ onBack }: { onBack: () => void }) {
  return <div className="space-y-5 animate-[content-enter_280ms_cubic-bezier(0.22,1,0.36,1)]" role="status" aria-live="polite" aria-busy="true">
    <span className="sr-only">Loading quotation details</span>
    <DetailPageBackButton label="Back to quotations" onClick={onBack} />

    <section className="relative overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_16px_46px_-34px_rgba(0,20,76,0.5)] sm:p-6" aria-hidden="true">
      <div className="pointer-events-none absolute inset-y-0 right-0 w-80 rounded-r-[1.5rem] bg-[radial-gradient(circle_at_100%_0%,rgba(82,56,168,0.1),transparent_62%)]" />
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0"><div className="flex items-center gap-2"><LoadingBlock className="h-2.5 w-20" /><LoadingBlock className="h-7 w-20 rounded-lg" /></div><LoadingBlock className="mt-3 h-8 w-48" /><LoadingBlock className="mt-3 h-4 w-56 max-w-full" /><LoadingBlock className="mt-2 h-3 w-72 max-w-full" /></div>
        <div className="flex flex-wrap items-center gap-2"><LoadingBlock className="h-10 w-24 rounded-xl" /><LoadingBlock className="h-10 w-36 rounded-xl" /><LoadingBlock className="h-10 w-24 rounded-xl" /></div>
      </div>
      <div className="relative mt-6 rounded-2xl border border-slate-200/80 bg-slate-50/65 px-4 py-3.5">
        <div className="flex items-center justify-between gap-4"><div className="space-y-2"><LoadingBlock className="h-2 w-20" /><LoadingBlock className="h-3 w-28" /></div><LoadingBlock className="h-7 w-20 rounded-lg" /></div>
        <div className="relative mt-4 h-4 px-1"><div className="absolute inset-x-2 top-1.5 h-1 overflow-hidden rounded-full bg-slate-200"><div className="h-full w-1/3 animate-pulse rounded-full bg-[linear-gradient(90deg,#10b981,#f48020)] motion-reduce:animate-none" /></div><div className="absolute inset-x-0 top-0 flex justify-between">{Array.from({ length: 4 }, (_, index) => <LoadingBlock className={`size-3 rounded-full ${index === 0 ? 'ring-4 ring-orange-100' : ''}`} key={index} />)}</div></div>
        <div className="mt-1 flex justify-between"><LoadingBlock className="h-2 w-12" /><LoadingBlock className="h-2 w-20" /></div>
      </div>
      <div className="relative mt-5 border-t border-slate-100 pt-4"><LoadingBlock className="h-3 w-2/3" /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{['bg-white', 'bg-emerald-50/55', 'bg-violet-50/55', 'bg-white'].map((tone, index) => <article className={`rounded-2xl border border-slate-200/80 p-4 ${tone}`} key={index}><LoadingBlock className="h-2.5 w-24" /><LoadingBlock className={`mt-3 h-6 ${index === 2 ? 'w-16' : 'w-28'}`} /></article>)}</div></div>
    </section>

    <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]" aria-hidden="true">
      <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]">
        <div className="flex items-center gap-3"><LoadingBlock className="size-9 rounded-xl" /><div><p className="text-sm font-extrabold text-brand-blue">Client &amp; project information</p><p className="mt-0.5 text-[10px] text-slate-400">Client and delivery details</p></div></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">{Array.from({ length: 4 }, (_, index) => <div className="rounded-xl bg-slate-50/70 p-3.5" key={index}><LoadingBlock className="h-2.5 w-20" /><LoadingBlock className={`mt-2.5 h-4 ${index === 2 ? 'w-4/5' : 'w-3/5'}`} /></div>)}</div>
      </section>
      <aside className="rounded-[1.5rem] bg-[linear-gradient(145deg,#00113f,#073078)] p-5 shadow-[0_20px_48px_-30px_rgba(0,20,76,0.75)]">
        <div className="flex items-center gap-3"><div className="size-9 animate-pulse rounded-xl bg-white/10 motion-reduce:animate-none" /><div className="space-y-2"><div className="h-3 w-32 animate-pulse rounded bg-white/30 motion-reduce:animate-none" /><div className="h-2.5 w-40 animate-pulse rounded bg-white/15 motion-reduce:animate-none" /></div></div>
        <div className="mt-5 space-y-3">{Array.from({ length: 3 }, (_, index) => <div className="flex justify-between gap-4" key={index}><div className="h-3 w-24 animate-pulse rounded bg-white/15 motion-reduce:animate-none" /><div className="h-3 w-20 animate-pulse rounded bg-white/30 motion-reduce:animate-none" /></div>)}</div>
        <div className="mt-5 flex items-end justify-between border-t border-white/15 pt-5"><div className="h-2.5 w-20 animate-pulse rounded bg-white/15 motion-reduce:animate-none" /><div className="h-7 w-32 animate-pulse rounded-lg bg-white/30 motion-reduce:animate-none" /></div>
      </aside>
    </div>

    <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]" aria-hidden="true">
      <header className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><p className="text-sm font-extrabold text-brand-blue">Quotation items</p><p className="mt-1 text-[10px] text-slate-400">All quoted items and prices</p></div><LoadingBlock className="h-7 w-16 rounded-lg" /></header>
      <div className="overflow-x-auto"><div className="min-w-[820px]"><div className="grid grid-cols-[6%_36%_12%_12%_17%_17%] bg-slate-50/70 px-4 py-3.5">{Array.from({ length: 6 }, (_, index) => <LoadingBlock className={`h-2.5 ${index === 1 ? 'w-20' : 'w-10'}`} key={index} />)}</div>{Array.from({ length: 4 }, (_, index) => <div className="grid grid-cols-[6%_36%_12%_12%_17%_17%] items-center border-t border-slate-100 px-4 py-4" key={index}><LoadingBlock className="mx-auto h-3 w-3" /><div className="flex items-center gap-3"><LoadingBlock className="size-11 shrink-0 rounded-xl" /><div className="min-w-0 flex-1 space-y-2"><LoadingBlock className={`h-3 ${index === 1 ? 'w-3/4' : 'w-1/2'}`} /><LoadingBlock className="h-2.5 w-1/3" /></div></div><LoadingBlock className="h-3 w-12" /><LoadingBlock className="ml-auto h-3 w-10" /><LoadingBlock className="ml-auto h-3 w-20" /><LoadingBlock className="ml-auto h-3 w-24" /></div>)}</div></div>
    </section>

    <div className="grid gap-5 lg:grid-cols-2" aria-hidden="true">
      <section className="rounded-[1.5rem] border border-emerald-100 bg-white p-5 shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]"><div className="flex items-center gap-3"><LoadingBlock className="size-9 rounded-xl" /><div><p className="text-sm font-extrabold text-brand-blue">Expected profit</p><p className="mt-0.5 text-[10px] text-slate-400">For internal use only</p></div></div><div className="mt-5 grid grid-cols-2 gap-3">{['bg-slate-50/70', 'bg-emerald-50/65', 'bg-violet-50/65', 'bg-blue-50/65'].map((tone, index) => <div className={`rounded-xl p-3.5 ${tone}`} key={index}><LoadingBlock className="h-2.5 w-24" /><LoadingBlock className={`mt-3 h-4 ${index === 2 ? 'w-14' : 'w-24'}`} /></div>)}</div></section>
      <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_14px_40px_-32px_rgba(0,20,76,0.35)]"><div className="flex items-center gap-3"><LoadingBlock className="size-9 rounded-xl" /><div><p className="text-sm font-extrabold text-brand-blue">History</p><p className="mt-0.5 text-[10px] text-slate-400">When this quotation changed</p></div></div><div className="mt-5 space-y-4">{Array.from({ length: 3 }, (_, index) => <div className="flex gap-3" key={index}><LoadingBlock className="mt-1 size-2 shrink-0 rounded-full ring-4 ring-slate-100" /><div className="flex-1 space-y-2"><LoadingBlock className={`h-3 ${index === 2 ? 'w-44' : 'w-28'}`} /><LoadingBlock className="h-2.5 w-40" /></div></div>)}</div></section>
    </div>
  </div>
}

function QuotationRouteLoading({ mode, onBack }: { mode: 'edit' | 'detail'; onBack: () => void }) {
  return mode === 'edit' ? <QuotationEditorLoading onBack={onBack} /> : <QuotationDetailLoading onBack={onBack} />
}
...
export function QuotationsPage({ currentUsername }: QuotationsPageProps) {
  ...
  useEffect(() => {
    ...
    void listQuotations({ pageSize: 100 }).then((result) => { setQuotations(result.items.map(toQuotation)); setHasLoadedQuotations(true); setStorageError('') }).catch(() => setStorageError('Quotations could not be loaded from the API.')).finally(() => setIsLoadingQuotations(false))
  ...
  const encodedEditRouteQuotationId = /^\/quotations\/([^/]+)\/edit$/.exec(currentPath)?.[1]
  const editRouteQuotationId = encodedEditRouteQuotationId ? decodeURIComponent(encodedEditRouteQuotationId) : undefined
  const editRouteQuotation = editRouteQuotationId ? quotations.find((quotation) => quotation.id === editRouteQuotationId && isActiveRecord(quotation) && quotation.status !== 'Approved') : undefined
  const approvalQuotation = approvalReviewId ? quotations.find((quotation) => quotation.id === approvalReviewId) : undefined

  useEffect(() => {
    if (!editRouteQuotation || (isFormOpen && editingId === editRouteQuotation.id)) return
    setDraft(quotationDraftFrom(editRouteQuotation, clients))
    setEditingId(editRouteQuotation.id)
    setSelectedId(null)
    setFormError('')
    setItemSearch('')
    setIsPricingDialogOpen(false)
    setIsFormOpen(true)
  }, [clients, editRouteQuotation, editingId, isFormOpen])
  ...
  if (editRouteQuotationId && !editRouteQuotation && !isFormOpen && isLoadingQuotations) return <QuotationRouteLoading mode="edit" onBack={backToQuotationRegister} />
  if (editRouteQuotationId && !editRouteQuotation && !isFormOpen && !hasLoadedQuotations) return <p className="rounded-xl border border-red-100 bg-red-50 p-4 text-xs font-semibold text-red-600" role="alert">{storageError}</p>
  if (editRouteQuotationId && !editRouteQuotation && !isFormOpen) return <div className="grid min-h-[28rem] place-items-center text-center"><div><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-amber-50 text-amber-500"><Icon className="size-6" path="M12 9v4m0 4h.01M10.3 3.9 2.2 18a2 2 0 0 0 1.7 3h16.2a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z" /></span><h2 className="mt-4 text-xl font-extrabold text-brand-blue">Quotation unavailable for editing</h2><p className="mt-2 text-xs text-slate-400">It may have been archived, removed, or locked after approval.</p><button className="mt-5 h-10 rounded-xl bg-brand-blue px-4 text-xs font-bold text-white" type="button" onClick={backToQuotationRegister}>Back to quotations</button></div></div>

  if (routeQuotationId && !routeQuotation && !isFormOpen && isLoadingQuotations) return <QuotationRouteLoading mode="detail" onBack={backToQuotationRegister} />
  if (routeQuotationId && !routeQuotation && !isFormOpen && !hasLoadedQuotations) return <p className="rounded-xl border border-red-100 bg-red-50 p-4 text-xs font-semibold text-red-600" role="alert">{storageError}</p>
