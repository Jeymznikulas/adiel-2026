...
export function QuotationsPage({ currentUsername }: QuotationsPageProps) {
  ...
  function approveQuotation(quotation: Quotation) {
    ...
    appendSystemLog({ recordId: quotation.id, module: 'Quotations', action: 'Status changed', entity: quotation.quotationNumber, description: `Quotation reviewed, approved, and locked for ${quotation.clientName}.`, actor: currentUsername, tone: 'success', amount: quotation.totalAmount, status: 'Approved' })
    setApprovalReviewId(null)
    setToast('Quotation approved and locked successfully.')
  }

  const approvalDialog = approvalQuotation ? <QuotationApprovalDialog quotation={approvalQuotation} onClose={() => setApprovalReviewId(null)} onEdit={() => { setApprovalReviewId(null); openEditQuotation(approvalQuotation) }} onApprove={() => approveQuotation(approvalQuotation)} /> : null
  ...
  return <div className="space-y-5 animate-[content-enter_360ms_cubic-bezier(0.22,1,0.36,1)]">
    <SummarySurface className="grid gap-5 xl:grid-cols-[1fr_auto] xl:items-center" aria-label="Quotation summary">
      <div><div className="flex items-center gap-2"><span className="h-px w-6 bg-brand-orange" /><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-orange">Commercial proposals</p></div><h2 className="mt-3 text-2xl font-bold tracking-[-0.04em] text-brand-blue sm:text-3xl">Quotations</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Prepare client proposals, monitor approvals, and keep estimated profitability visible before work begins.</p></div>
