...
function QuotationDetailLoading({ onBack }: { onBack: () => void }) {
  return <section className="overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[0_24px_70px_-45px_rgba(0,20,76,0.45)] animate-[content-enter_280ms_cubic-bezier(0.22,1,0.36,1)]" role="status" aria-live="polite" aria-busy="true"><span className="sr-only">Loading quotation details</span><div className="h-1 bg-[linear-gradient(90deg,#00144c,#5b7fe8,#fd4d00)] opacity-80" /><header className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7"><div className="flex items-start gap-3.5"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-brand-blue text-white"><Icon className="size-5" path="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6M8 13h8M8 17h5" /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-brand-orange">Quotation details</p><h2 className="mt-1.5 text-xl font-extrabold text-brand-blue">Opening quotation…</h2><p className="mt-1 text-xs text-slate-400">Gathering the record, status, and financial summary.</p></div></div><button className="h-9 self-start rounded-xl px-3 text-[10px] font-bold text-slate-500 hover:bg-slate-50 hover:text-brand-blue" type="button" onClick={onBack}>Back</button></header><div className="space-y-5 bg-slate-50/35 p-4 sm:p-6" aria-hidden="true"><div className="grid gap-3 lg:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <div className={`rounded-2xl border border-slate-200 bg-white p-4 ${index === 0 ? 'lg:col-span-2' : ''}`} key={index}><LoadingBlock className="h-2.5 w-16" /><LoadingBlock className="mt-3 h-4 w-3/4" /><LoadingBlock className="mt-2 h-2.5 w-1/2" /></div>)}</div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">{Array.from({ length: 4 }, (_, index) => <div className="flex items-center gap-4 border-b border-slate-100 p-4 last:border-b-0" key={index}><LoadingBlock className="size-10 shrink-0 rounded-xl" /><LoadingBlock className="h-3 flex-1" /><LoadingBlock className="h-3 w-24" /><LoadingBlock className="h-3 w-28" /></div>)}</div></div></section>
}
