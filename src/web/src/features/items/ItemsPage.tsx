  ...
  return (
    <div className="space-y-5 animate-[content-enter_360ms_cubic-bezier(0.22,1,0.36,1)]">
      ...
      <section className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_14px_45px_-30px_rgba(0,20,76,0.28)]" aria-labelledby="items-list-heading">
        ...
        {visibleItems.length ? <div className="overflow-x-auto"><table className="w-full min-w-[820px] table-fixed text-left"><thead><tr className="border-b border-slate-100 bg-slate-50/70 text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400"><th className="w-[11%] px-5 py-3.5">Photo</th><th className="w-[28%] px-4 py-3.5">Product name</th><th className="w-[19%] px-4 py-3.5">Product category</th><th className="w-[17%] px-4 py-3.5">Unit of measure</th><th className="w-[12%] px-4 py-3.5 text-center">Variations</th><th className="w-[13%] px-5 py-3.5 text-right">Details</th></tr></thead><tbody>
          {visibleItems.map((item) => <tr className="group border-b border-slate-100 transition-colors hover:bg-[#fbfcfe]" key={item.id}>
            <td className="border-l-4 border-l-brand-orange px-5 py-4"><ProductPhoto item={item} size="small" /></td>
            <td className="px-4 py-4"><button className="block max-w-full truncate text-left text-sm font-bold text-brand-blue transition hover:text-brand-orange" type="button" onClick={() => openItemDetails(item)}>{item.name}</button></td>
            <td className="px-4 py-4"><span className="inline-flex max-w-full rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs font-bold text-brand-blue"><span className="truncate">{item.category}</span></span></td>
            <td className="px-4 py-4 text-sm font-semibold text-slate-600">{item.unitOfMeasure}</td>
            <td className="px-4 py-4 text-center"><span className="inline-grid min-w-9 place-items-center rounded-lg bg-violet-50 px-2.5 py-1.5 text-xs font-extrabold text-violet-700">{item.variants.length}</span></td>
            <td className="px-5 py-4 text-right"><button className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-[11px] font-bold text-brand-blue shadow-sm transition hover:-translate-y-0.5 hover:border-brand-blue/20 hover:bg-blue-50" type="button" onClick={() => openItemDetails(item)}>Full details<Icon className="size-3.5" path="m9 18 6-6-6-6" /></button></td>
          </tr>)}
        </tbody></table></div> : <div className="grid min-h-80 place-items-center p-8 text-center"><div className="max-w-sm"><span className="mx-auto grid size-16 place-items-center rounded-2xl border border-slate-200 bg-slate-50 text-brand-blue shadow-[0_15px_35px_-26px_rgba(0,20,76,0.5)]"><Icon className="size-6" path="m21 8-9-5-9 5 9 5 9-5ZM3 12l9 5 9-5M3 16l9 5 9-5" /></span><p className="mt-5 text-[9px] font-bold uppercase tracking-[0.15em] text-brand-orange">{items.length ? 'No matches' : 'Build your catalog'}</p><h3 className="mt-2 text-lg font-bold text-brand-blue">{items.length ? 'No items found' : 'Add your first item'}</h3><p className="mt-2 text-xs leading-5 text-slate-400">{items.length ? 'Try another search or clear the active filters.' : suppliers.length ? 'Create a product record with specifications, supplier sourcing, and pricing.' : 'Add a supplier first, then return here to create your product catalog.'}</p>{items.length ? <button className="mt-5 h-10 rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-500" type="button" onClick={() => { setSearch(''); setCategoryFilter('All categories'); setStatusFilter('All statuses') }}>Clear filters</button> : suppliers.length ? <button className="mt-5 h-10 rounded-xl bg-brand-blue px-4 text-xs font-bold text-white" type="button" onClick={openAddDialog}>Add first item</button> : <a className="mt-5 inline-flex h-10 items-center rounded-xl bg-brand-blue px-4 text-xs font-bold text-white" href="/suppliers">Open Supplier Directory</a>}</div></div>}
      </section>
