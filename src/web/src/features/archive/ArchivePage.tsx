...
export function ArchivePage({ currentUsername: _currentUsername }: { currentUsername: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { void listArchive({ search, page, pageSize: 30 }).then((result) => { if (!active) return; setItems(result.items); setTotal(result.total); setHasLoadedArchive(true); setError('') }).catch((failure: unknown) => { if (active) setError(failure instanceof Error ? failure.message : 'Archive could not be loaded.') }).finally(() => { if (active) setIsLoadingArchive(false) }) }, 160)
