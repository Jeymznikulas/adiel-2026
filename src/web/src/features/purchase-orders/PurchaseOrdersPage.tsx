...
import { PurchaseOrderCompanyDetailsDialog } from './PurchaseOrderCompanyDetailsDialog'
...
import { defaultPurchaseOrderTerms } from './purchaseOrderDocumentDefaults'
...
function loadPurchaseOrders(): PurchaseOrder[] {
  try {
    ...
    return parsed.flatMap((value) => {
      ...
      return [{ ...(order as PurchaseOrder), clientId: typeof order.clientId === 'string' ? order.clientId : '', quotationId: typeof order.quotationId === 'string' ? order.quotationId : '', quotationNumber: typeof order.quotationNumber === 'string' ? order.quotationNumber : '', notes: typeof order.notes === 'string' ? order.notes : '', terms: typeof order.terms === 'string' && order.terms.trim() ? order.terms : defaultPurchaseOrderTerms, items, subtotalAmount, vatEnabled, vatAmount, otherCharges, totalAmount }]
    })
  } catch { return [] }
}

function emptyDraft(): PurchaseOrderDraft {
  return { date: new Date().toISOString().slice(0, 10), poNumber: '', clientId: '', clientName: '', supplierId: '', contactPerson: '', subject: '', quotationId: '', quotationNumber: '', modeOfPayment: 'Bank transfer', paymentTerm: '30 days', deliveryLocation: '', modeOfDelivery: 'Supplier delivery', notes: '', terms: defaultPurchaseOrderTerms, items: [], vatEnabled: false, otherCharges: [], status: 'Not yet sent' }
}
...
export function PurchaseOrdersPage({ currentUsername }: PurchaseOrdersPageProps) {
  ...
  useEffect(() => {
    if (!isFormOpen && !selectedOrderId && !isDocumentDialogOpen && !isCompanyDetailsOpen && !isClientPickerOpen) return
    const overflow = document.body.style.overflow
    document.body.style.overflow = 'hidden'
    return () => { document.body.style.overflow = overflow }
  }, [isClientPickerOpen, isCompanyDetailsOpen, isDocumentDialogOpen, isFormOpen, selectedOrderId])
  ...
  return <div className="purchase-order-page space-y-5 animate-[content-enter_360ms_cubic-bezier(0.22,1,0.36,1)]">
    ...
    <section className="flex flex-col gap-4 overflow-hidden rounded-2xl border border-brand-blue/10 bg-[linear-gradient(110deg,rgba(0,20,76,0.045),rgba(255,255,255,0.98),rgba(244,128,32,0.055))] p-4 shadow-[0_12px_32px_-28px_rgba(0,20,76,0.55)] sm:flex-row sm:items-center sm:justify-between sm:px-5">
      <div className="flex min-w-0 items-center gap-3.5"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-brand-blue text-white shadow-[0_10px_22px_-12px_rgba(0,20,76,0.8)]"><Icon className="size-5" path="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6" /></span><div className="min-w-0"><p className="text-sm font-extrabold text-brand-blue">Own company document details</p><p className="mt-1 text-[11px] leading-5 text-slate-500">Address, office contacts, email, and TIN used automatically in every purchase-order PDF.</p></div></div>
      <button className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-brand-blue/10 bg-white px-4 text-xs font-bold text-brand-blue shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-blue/20 hover:bg-blue-50 hover:shadow-md" type="button" onClick={() => setIsCompanyDetailsOpen(true)}><Icon className="size-4" path="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z" />Edit company details</button>
    </section>
    ...
    {isCompanyDetailsOpen ? <PurchaseOrderCompanyDetailsDialog onSaved={() => setToast('Company document details saved')} onClose={() => setIsCompanyDetailsOpen(false)} /> : null}
