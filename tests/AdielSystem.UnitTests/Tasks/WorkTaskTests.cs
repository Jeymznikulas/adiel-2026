...
public sealed class WorkTaskTests
{
    ...
    [Fact]
    public void Task_details_are_normalized_and_priority_and_due_date_are_retained()
    {
        var dueDate = new DateOnly(2026, 8, 30);
        var task = WorkTask.Create(Guid.NewGuid(), "  Prepare quote  ", "  Confirm pricing  ", WorkTaskPriority.High, null, "  Alex Morgan  ", OwnerId, "owner", dueDate, Now);
    ...
    {
        Assert.Throws<ArgumentException>(() => WorkTask.Create(Guid.NewGuid(), "Task", "", WorkTaskPriority.Medium, null, "Alex", OwnerId, "owner", new DateOnly(2026, 8, 25), Now));
    }
    ...
    public void Task_cannot_complete_while_a_subtask_is_incomplete()
    {
        ...
        var task = WorkTask.Rehydrate(subtask.TaskId, "Task", "", WorkTaskStatus.InProgress, WorkTaskPriority.Medium, null, "Alex", OwnerId, "owner", null, null, Now, Now, null, 1, [subtask]);
    ...
    private static WorkTask CreateTask() => WorkTask.Create(Guid.NewGuid(), "Task", "", WorkTaskPriority.Medium, null, "Alex", OwnerId, "owner", null, Now);
}
